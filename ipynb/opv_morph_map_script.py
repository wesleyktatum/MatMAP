...
for i,fl in enumerate(files):
    ims.append(np.load(test_file_path+fl))
...
#    outliers = pre.extract_outliers(data, data_type, threshold = outlier_threshold)
    ...
    for prop in data_properties:
        temp_outliers = pre.extract_outliers(data, data_type, prop, 2.5, 256, 256)
...
padding = 0
embedding_dim = 4
thresh = 0.1     ## Persistence Watershed Threshold
nonlinear = True
normalize = True
zscale = False
data_type = 'Full_QNM'
...
# Access all of the uniquely labeled domains in the image and measure them.
# Then, label and sort the associated measurements tabels with the majority
# phase label. Plot the results first sorted by phase label, then simply 
# by domain label.

#    metrics = slu.all_domain_properties(seg1_labels, seg2_labels)
#    print ('Sorted by GMM label')
#    slu.plot_descriptors_by_phase(metrics)
#    print('Sorted by domain label')
#    slu.plot_descriptors_by_domain(metrics)
#     
#    #boxplot of results, rather than individual points. phase sorted.
#    fig, axs = plt.subplots(1,5,figsize = (20,8),sharey=False)
#    metrics.boxplot(['major_axis_length', 'minor_axis_length', 'perimeter', 'orientation', 'eccentricity'], by = 'label', ax = axs)
#    
#    metrics = pd.DataFrame(metrics)
#    metrics.head()
#    
#    avg_MAL = np.average(metrics['major_axis_length'])
#    std_MAL = np.std(metrics['major_axis_length'])
#    print (avg_MAL)
#    print (std_MAL) 
...
#########################################################################    
#    phase0 = np.asarray([x for x in seg1_labels == 0])
#    phase1 = np.asarray([x for x in seg1_labels == 1])
#    phase2 = np.asarray([x for x in seg1_labels == 2])
#    phase3 = np.asarray([x for x in seg1_labels == 3])
#    
#    plt.imshow(phase0)
#    plt.title('Phase 0')
#    plt.show()
...
#    plt.show()
#    
#    domain_labels = np.asarray(seg2_labels)
#    
#    plt.imshow(domain_labels, cmap = 'rainbow')
#    x,y = domain_labels.shape
#    print (x,y)
#    
#    phase1_domains = np.zeros((x,y))
#    
#    phase1_domains[phase1 == True] = domain_labels[phase1 == True]
#    
#    print (phase1_domains.shape)
#    print (phase1_domains.max())
#    plt.imshow(phase1_domains, cmap = 'gist_ncar')
#    
#    plt.show()
#    plt.imshow(phase1)
#    plt.show()
#    
#    remapped_labels = np.zeros([x,y])
#    unique_labels = slu.get_unique_labels(phase1_domains)
#    unique_labels.sort()
#    print (unique_labels)
#    
#    
...
#    for i in range(x):
#        for j in range(y):
#            if phase1_domains[i,j] != 0:
#                element = phase1_domains[i,j]
#                remapped_labels[i,j] = unique_labels.index(element)
#            else:
#                remapped_labels[i,j] = 0
#    #             pass
#            
#        
#    
#    print (remapped_labels.max(), ' different domains in this phase')
#    
#    num_labels = int(remapped_labels.max())
#    input_cmap = 'nipy_spectral'
#    cmap = plt.get_cmap(input_cmap, num_labels)
#    plt.imshow(remapped_labels, cmap = cmap)
#    post.colorbar_index(num_labels, cmap = cmap)
#    
#    plt.show()
#    
#    plot = [x for x in remapped_labels != 0]
#    plt.imshow(plot)
#    plt.title('Phase 1 regions with PWS labels')
#    plt.show()
#    
#    print (remapped_labels.shape)
#        
#    seg2_labels = seg2_dict[0]
#    plt.imshow(seg2_labels)
#    plt.title('PWS labels')
#    plt.show()
#    
#    seg2_phase1_domains = np.zeros([x,y])
#    seg2_phase1_domains[remapped_labels != 0] = seg2_labels[remapped_labels != 0]
#    plt.imshow(seg2_phase1_domains)
#    plt.title('PWS labels with phase 1 labels')
#    plt.show()
#    
#    ## The difference between the last two above is the domains associated with phase 1 labels, that were outside of phase 1 regions
#    
#    label_diff = seg2_labels - np.where(seg2_phase1_domains>0,0,seg2_phase1_domains.max())
#    
#    plt.imshow(label_diff)
#    plt.title('Phase 1 labeled domains, with PWS domain label')
#    
#    plt.show()
#    
#    for i in range(int(remapped_labels.max())):
#        print ('Index: ',i)
#        plot = [x for x in remapped_labels == i]
#        plt.imshow(plot)
#        plt.show()
#    # plt.show()     
#############################################################################